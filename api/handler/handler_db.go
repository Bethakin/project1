...
func (h *TodoHandlerDB) Index2(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		rows.Scan(&todo.ID, &todo.Email, &todo.Password)
...
func (h *TodoHandlerDB) Index(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := h.DB.Query("SELECT id, email, password FROM todos WHERE id = $1", id)
	defer rows.Close()
	h.DB.Exec("SELECT id, email, password FROM todos WHERE id = $1", id)
